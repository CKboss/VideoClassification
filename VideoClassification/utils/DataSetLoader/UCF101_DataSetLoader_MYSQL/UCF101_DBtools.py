...
def getFrames_imgfilepath(splitkind='test'):
    ...
    :return: 连续的原始图片
    '''
    sql1 = 'SELECT imgname,videoname,ord FROM ImgSets WHERE splitkind="{}" and imgkind="frame" ORDER BY RAND(),videoname,RAND(),ord LIMIT 1;'.format(
        splitkind)
    ...
    sql2 = 'SELECT imgpath,first_label FROM ImgSets WHERE videoname="{}" and splitkind="{}" and imgkind="frame" and ord>={} and ord<{} ORDER BY ord;'.format(
        videoname, splitkind, ord, ord + 50)
...
def getTemporals_imgfilepath(splitkind='test'):
    sql1 = 'SELECT imgname,videoname,ord FROM ImgSets WHERE splitkind="{}" and imgkind="flow" ORDER BY RAND(),videoname,RAND(),ord LIMIT 1;'.format(
        splitkind)
    ...
    sql2 = 'SELECT imgpath,first_label FROM ImgSets WHERE videoname="{}" and splitkind="{}" and imgkind="flow" and ord>={} and ord<{} ORDER BY ord;'.format(
        videoname, splitkind, ord, ord + 500)
    cursor.execute(sql2)
    res = cursor.fetchall()
...
def getMixs_imgfilepath(splitkind='test'):
    sql1 = 'SELECT imgpath,videoname,ord FROM ImgSets WHERE splitkind="{}" and imgkind="frame" ORDER BY RAND(),videoname,RAND(),ord LIMIT 1;'.format(
        splitkind)
    ...
    sql2 = 'SELECT imgpath,first_label FROM ImgSets WHERE videoname="{}" and imgkind="flow" and ord>{} and ord <{} ORDER BY ord;'.format(
        videoname, ordl, ordr)
