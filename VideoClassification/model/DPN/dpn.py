...
def dpn92(num_classes=101, in_channel=3):
    model = DPN(num_init_features=64, k_R=96, G=32, k_sec=(3, 4, 20, 3), inc_sec=(16, 32, 24, 128),
                num_classes=num_classes, in_channel=in_channel)
    return nn.DataParallel(model)


def dpn98(num_classes=101, in_channel=3):
    model = DPN(num_init_features=96, k_R=160, G=40, k_sec=(3, 6, 20, 3), inc_sec=(16, 32, 32, 128),
                num_classes=num_classes, in_channel=in_channel)
    return nn.DataParallel(model)


def dpn107(num_classes=101, in_channel=3):
    model = DPN(num_init_features=128, k_R=160, G=40, k_sec=(4, 8, 28, 3), inc_sec=(16, 32, 32, 128),
                num_classes=num_classes, in_channel=in_channel)
    return nn.DataParallel(model)


def dpn131(num_classes=101, in_channel=3):
    model = DPN(num_init_features=128, k_R=200, G=50, k_sec=(4, 8, 20, 3), inc_sec=(20, 64, 64, 128),
                num_classes=num_classes, in_channel=in_channel)
    return nn.DataParallel(model)
...
class DualPathBlock(nn.Module):
    def __init__(self, in_chs, num_1x1_a, num_3x3_b, num_1x1_c, inc, G, _type='normal'):
        ...
        self.layers = nn.Sequential(OrderedDict([
            ...
            ('c3x3_b',
             self.BN_ReLU_Conv(in_chs=num_1x1_a, out_chs=num_3x3_b, kernel_size=3, stride=key_stride, padding=1,
                               groups=G)),
...
class DPN(nn.Module):
    def __init__(self, num_init_features=64, k_R=96, G=32,
                 k_sec=(3, 4, 20, 3), inc_sec=(16, 32, 24, 128), num_classes=1000, in_channel=3):
        ...
        blocks['conv1'] = nn.Sequential(
            nn.Conv2d(in_channel, num_init_features, kernel_size=7, stride=2, padding=3, bias=False),
