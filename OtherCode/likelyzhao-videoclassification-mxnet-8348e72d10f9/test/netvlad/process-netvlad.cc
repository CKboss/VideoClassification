...
void GetImageFile(const std::string image_file,
    ...
    std::string fea_name = "/workspace/data/trainval/" + image_file + "_flatten_imagenet22k_frame.binary";
...
//    std::cout<< image_file <<std::endl;
    ...
    int fealen = filesize/sizeof(float)/FEATURE_LEN;
    for (int i =0;i<NUM_INPUT;i++){
        int randidx = random(fealen);
        memcpy(image_data+ i* FEATURE_LEN,tempfea + randidx*FEATURE_LEN,sizeof(float)*FEATURE_LEN);
	}
}
...
int main(int argc, char* argv[]) {
    ...
    std::string param_file = "model/netvlad-0080.params";
    ...
    const mx_uint input_shape_indptr[2] = { 0, 3 };
    const mx_uint input_shape_data[4] = { 1,
                                        static_cast<mx_uint>(NUM_INPUT), 
                                        static_cast<mx_uint>(FEATURE_LEN)};
    ...
    int image_size = FEATURE_LEN * NUM_INPUT;
	...
	GetImageFile(filelist[i], image_data.data());
