...
int fusionscore(std::vector<float> scores,std::vector<int> predidx,int shape_len,int & outidx,float & outscore){
    if (scores.size() != predidx.size()) {
        std::cerr << "Result data and socre size does not match!" << std::endl;
    }
    int cum_idx[shape_len];
    float  max_score[shape_len];
    std::cout<<"shape len"<<shape_len<<std::endl;
    for (int i=0;i<shape_len;i++) cum_idx[i]=0;
    for (int i=0;i<shape_len;i++) max_score[i]=0.0;
    for (int i =0;i<scores.size();i++){
    	if (scores[i] > 0.6){
  //          std::cout<<"idx"<<predidx[i]<<"score"<<scores[i]<<std::endl;
	    cum_idx[predidx[i]]++;
            if (scores[i] > max_score[predidx[i]]) max_score[predidx[i]]=scores[i]; 
	}
    }
    int best_cum=0;
    int best_idx=0;
    float best_score =max_score[0];
    for (int i =1;i<shape_len;i++){
	if (cum_idx[i] >  cum_idx[best_idx]){
	    best_idx = i;
            best_score = max_score[i];
            std::cout<<"idx"<<best_idx<<"score"<<best_score<<std::endl;
	}
	if ((cum_idx[i] ==  cum_idx[best_idx])&&(max_score[i]>best_score)){
	    best_idx = i;
            best_score = max_score[i];
	}
    }	
...
int main(int argc, char* argv[]) {
    ...
    std::string json_file = "/workspace/data_recordio/code/videoclassification-mxnet/train/model/UCF-symbol.json";
    std::string param_file = "/workspace/data_recordio/code/videoclassification-mxnet/train/model/UCF-0010.params";
    std::string synset_file = "/workspace/data_recordio/code/videoclassification-mxnet/train/model/synsets.txt";
    std::string nd_file = "";
    ...
    for (mx_uint i =0;i<filelist.size();i++){ 
        ...
        for (mx_uint j =0; j< temp.size();j++){
  //          std::cout<<temp[j]<<std::endl;
	    GetImageFile(temp[j], image_data.data(),
	    ...
	    float best_accuracy = 0.0;
    	    ...
    	    for (int ii = 0; ii < static_cast<int>(data.size()); ii++) {
        	if ( data[ii] > best_accuracy ) {
            	    best_accuracy = data[ii];
            	    best_idx = ii;
        	}
    	    }
           
// 	    printf("Accuracy[%d] = %.8f\n", best_idx, best_accuracy);
            score.push_back(best_accuracy);
            predidx.push_back(best_idx);
	}
        int labelPred;
        float score_max;
        if (fusionscore(score,predidx,size,labelPred,score_max) == 0){
   //        std::cout<<"label Pred"<<labelPred<<"score"<< score_max<<std::endl;
            if (labelPred == atoi(labels[i].c_str())){
		 right++;
		 std::cout<<"right:"<<right<<"/"<<(i+1)<<std::endl;
            }
        }
