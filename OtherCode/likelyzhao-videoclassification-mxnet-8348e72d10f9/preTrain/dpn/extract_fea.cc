...
const mx_float DEFAULT_MEAN_R = 124;
const mx_float DEFAULT_MEAN_G = 117;
const mx_float DEFAULT_MEAN_B = 104;
const std::string OUTPUTLAYERNAME = "flatten";
...
//    std::cout<< image_file <<std::endl;
    ...
    cv::Mat im256;
    float resize_ratio;
//    std::cout<<"im rows :"<<im_ori.rows<<std::endl;  
...
//    resize(im_ori, im, resize_size);
    ...
    for (int i = 0; i < im.rows; i++) {
        ...
        for (int j = 0; j < im.cols; j++) {
            ...
            if (channels > 1) {
                *ptr_image_g++ = (static_cast<mx_float>(*data++) - mean_g)*0.0167;
                *ptr_image_b++ = (static_cast<mx_float>(*data++) - mean_b)*0.0167;
            }

            *ptr_image_r++ = (static_cast<mx_float>(*data++) - mean_r)*0.0167;
        }
    }
}
...
int main(int argc, char* argv[]) {
    ...
    std::string json_file = "model/dpn92-extra-symbol.json";
    std::string param_file = "model/dpn92-extra-0000.params";
...
//                dev_type,//                 dev_id,
   ...
   //             &pred_hnd);
    ...
    for (mx_uint i =0;i<filelist.size();i++){ 
        ...
        std::vector<std::vector<float> > videoFea;
  ...
  //          std::cout<<temp[j]<<std::endl;
	    ...
	    // Do Predict Forward
   	    MXPredForward(pred_hnd);
            ...
            videoFea.push_back(data);
	  
	}
        //lsvc075999_fc6_vgg19_frame.binary
        std::string outputfilename = fea_path + label + "_" + OUTPUTLAYERNAME + "_imagenet22k_frame.binary";
        std::cout<< outputfilename << std::endl;
        FILE * fout = fopen(outputfilename.c_str(),"w");
        for (mx_uint ii =0;ii<videoFea.size();ii++)
            for (mx_uint jj =0;jj<videoFea[ii].size();jj++){
            fwrite(&videoFea[ii][jj],sizeof(float),1,fout);
	}
        fclose(fout);
 //       lsvc075999_fc6_vgg19_frame.binary
